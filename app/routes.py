...
def index(request: Request):
    ...
    batches = [{"name": name, "count": batch_counts.get(name, 0)} for name, _ in BATCH_PREFIXES]
    return templates.TemplateResponse(
        request,
        "index.html",
        {"batches": batches, "latest_sync": latest_sync, "image_count": DEFAULT_IMAGE_COUNT},
    )
...
def get_run_images(run_id: str):
    ...
    try:
        with get_conn() as conn:
            images = ensure_run_images(conn, run_id)
